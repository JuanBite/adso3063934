  ...
  <ul>
    ...
    <li>
      <span class="inline-flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="size-5" fill="white" viewBox="0 0 256 256"><path d="M256,136a8,8,0,0,1-8,8H232v16a8,8,0,0,1-16,0V144H200a8,8,0,0,1,0-16h16V112a8,8,0,0,1,16,0v16h16A8,8,0,0,1,256,136Zm-57.87,58.85a8,8,0,0,1-12.26,10.3C165.75,181.19,138.09,168,108,168s-57.75,13.19-77.87,37.15a8,8,0,0,1-12.25-10.3c14.94-17.78,33.52-30.41,54.17-37.17a68,68,0,1,1,71.9,0C164.6,164.44,183.18,177.07,198.13,194.85ZM108,152a52,52,0,1,0-52-52A52.06,52.06,0,0,0,108,152Z"></path></svg>
        Add User
      </span>
    </li>
  </ul>
        ...
        <div class="flex flex-col md:flex-row gap-4 mt-4">
            <div class="w-full md:w-[320px]">
                <div class="avatar flex flex-col gap-2 cursor-pointer justify-center items-center hover:scale-110 transition ease-in">
                    ...
                    @error('photo') <small class="badge badge-outline badge-error w-full py-4">{{ $message }}</small> @enderror
                </div>
            ...
            <div class="w-full md:w-[320px]">
                ...
                <input type="text" class="input bg-[#0006]" name="document" placeholder="1054123123"
                       value="{{ old('document') }}" />
                @error('document') <small class="badge badge-outline badge-error w-full py-4">{{ $message }}</small> @enderror
                ...
                <input type="text" class="input bg-[#0006]" name="fullname" placeholder="Jeremias Springfield"
                       value="{{ old('fullname') }}" />
                @error('fullname') <small class="badge badge-outline badge-error w-full py-4">{{ $message }}</small> @enderror
                ...
                @error('gender') <small class="badge badge-outline badge-error w-full py-4">{{ $message }}</small> @enderror

                <label class="label text-white">Birthdate</label>
                <input type="date" class="input bg-[#0006]" name="birthdate" value="{{ old('birthdate') }}" />
                @error('birthdate') <small class="badge badge-outline badge-error w-full py-4">{{ $message }}</small> @enderror
            </div>
            ...
            <div class="w-full md:w-[320px]">
                ...
                <input type="text" class="input bg-[#0006]" name="phone" placeholder="3051231234"
                       value="{{ old('phone') }}" />
                @error('phone') <small class="badge badge-outline badge-error w-full py-4">{{ $message }}</small> @enderror

                <label class="label text-white">Email</label>
                <input type="text" class="input bg-[#0006]" name="email" placeholder="Email"
                       value="{{ old('email') }}" />
                @error('email') <small class="badge badge-outline badge-error w-full py-4">{{ $message }}</small> @enderror

                <label class="label text-white">Password</label>
                <input type="password" class="input bg-[#0006]" name="password" placeholder="Password" />
                @error('password') <small class="badge badge-outline badge-error w-full py-4">{{ $message }}</small> @enderror

                <label class="label text-white">Password Confirmation</label>
                <input type="password" class="input bg-[#0006]" name="password_confirmation" placeholder="Confirm Password" />
                @error('password_confirmation') <small class="badge badge-outline badge-error w-full py-4">{{ $message }}</small> @enderror
            </div>
